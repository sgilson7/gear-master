...
| armor | temporary hit points; **starts every fight at zero**, granted per activation (roughly 5 per second of an item's cooldown), and soaks damage before health |
...
**Mana buffs.** Five components spend mana for a stacking buff that scales off
the mana you are *still holding*:

| buff | effect |
|---|---|
| **Mana Empowerment** | +0.05x weapon power per stack, per point of current mana |
| **Mana Shield** | −1 to every incoming hit per stack, per point of current mana — any damage type, applied before armour |

The catch is deliberate: a stack costs mana, so paying your whole pool for one
leaves it worth nothing. They only pay off with income above the cost. One
mana source feeding one empowerment piece produces a permanent +0.00x.
